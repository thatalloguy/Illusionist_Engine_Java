...
public class ProjectManager {
			 ...
			 try {
				...
				// convert users list to JSON file
				 ...
				 for (Entity entity: entities) {
					 System.out.println(entity.type + " | " + entity.name + " | " + entity.isRendered);
					 if (entity.type.startsWith("E") && entity.name.startsWith("L") == false && entity.isRendered == true) {
	...
	public void Open(ECS ecs) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				loadProject("Projects/" + currentProject + "/"+ currentProject + ".json", ecs, this.console);
	...
	public void loadProject(String path, ECS ecs, Console console) throws IOException {
		...
		gizmo.reload();
			...
			for (SaveEntity entity: entities) {
				Entity newEntity = entity.ConvertBack(ecs, console);
	...
	private void SetupProject(String name) throws IOException {
		...
		File scriptf = new File("Projects/" + name + "/Scripts");
		if (scriptf.mkdir()) {
			console.addMessage("Sucsessfully made the Script folder of: " + name);
		} else {
			console.addError("Couldn't make the Resource folder of: " + name);
		}
