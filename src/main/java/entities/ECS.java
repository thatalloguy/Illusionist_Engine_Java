...
public class ECS {
	...
	public List<gameScript> gameScripts = new ArrayList<gameScript>();
	...
	public DisplayManager Time = new DisplayManager();
	public SoundManager SoundManager = new SoundManager();
	private Boolean isCreatingScript = false;
	public Entity current_gizmo;
	public Boolean isPasted = false;
	private ImString scriptInput = new ImString();
	public Entity cameraEntity;
	public Game game;
	public ProjectManager pm;
	public gameScript currentGameScript;
	public entityScript currentScript;
	...
	public ECS(Console console, Game game, KeyBoard keyboard, Mouse mouse) {	
		this.console = console;
		this.game = game;
		this.keyboard = keyboard;
		this.mouse = mouse;
		this.CM = new ComponentManager(worldManager, console);
	}
	...
	public void RenderInfo() {
		...
		if (this.selected_entity != null & this.selected_light == null && selected_entity.type.startsWith("E")) {
			...
			if (ImGui.treeNodeEx("Entity Component", imgui.flag.ImGuiTreeNodeFlags.DefaultOpen)) {
				...
				if (ImGui.treeNodeEx("Scripts", imgui.flag.ImGuiTreeNodeFlags.DefaultOpen)) {
					this.resetTextColor();
					if (ImGui.button("New Script")) {
						if (this.pm.currentProject != null) {
							this.isCreatingScript = true;
						} else {
							console.addError("You cant make a script because you need a project!");
						}
					}
					
					if (this.isCreatingScript) {
						ImGui.inputText("Script Name", scriptInput);
						...
						if (ImGui.button("Done")) {
							this.isCreatingScript = false;
							try {
								entityScript script = new entityScript("Projects/" + pm.currentProject + "/Scripts/" + this.scriptInput.get() + ".groovy");
								script.setupScript();
								this.selected_entity.scripts.add(script);
					...
					if (!this.selected_entity.scripts.isEmpty()) {
						for (entityScript script : this.selected_entity.scripts) {
							if (ImGui.button(script.getPath())) {
								this.currentScript = script;
							}
							ImGui.sameLine();
							if (ImGui.button("delete Entity")) {
								if (this.currentScript != null && this.currentScript == script) {
									this.currentScript = null;
									
								}
								this.selected_entity.scripts.remove(script);
								break;
							}
						}
					}
		...
		if (this.selected_entity != null) {
			...
			if (ImGui.treeNodeEx("Components", ImGuiTreeNodeFlags.DefaultOpen)) {
				ImGui.text(" ");
				if (ImGui.button("Add Component", Utils.getPrecentageOf(19, DisplayManager.getWidth()), 20)) {
					CM.showComp();
				}


				int i = 0;
				for (Component component : this.selected_entity.components) {
